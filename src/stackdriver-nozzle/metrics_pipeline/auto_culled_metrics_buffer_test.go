...
var _ = Describe("autoCulledMetricsBuffer", func() {
	...
	It("culls duplicate metrics", func() {
		subject := NewAutoCulledMetricsBuffer(context.TODO(), logger, 100*time.Millisecond, metricAdapter, counter)
		...
		Expect(counter.GetCount("metrics.events.sampled")).To(Equal(1))
	})
	...
	It("culls multiple duplicates, keeping the latest", func() {
		...
		Expect(counter.GetCount("metrics.events.sampled")).To(Equal(2))
	})

	It("it buffers metrics for the expected duration before flushing", func() {
		d := 500 * time.Millisecond
		subject := NewAutoCulledMetricsBuffer(context.TODO(), logger, d, metricAdapter, counter)
	...
	It("it flushes metrics when the context is canceled", func() {
		...
		subject := NewAutoCulledMetricsBuffer(ctx, logger, d, metricAdapter, counter)
	...
	Describe("with a slow MetricAdapter", func() {
		...
		BeforeEach(func() {
			...
			subject = NewAutoCulledMetricsBuffer(context.TODO(), logger, 1*time.Millisecond, metricAdapter, counter)
		})
