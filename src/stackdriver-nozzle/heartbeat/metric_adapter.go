...
func (ma *metricAdapter) CreateMetricDescriptor(m Metric) error {
	projectName := path.Join("projects", ma.projectID)
	metricType := path.Join("custom.googleapis.com", m.Name)
	...
	req := &monitoringpb.CreateMetricDescriptorRequest{
		...
		MetricDescriptor: &metricpb.MetricDescriptor{
			...
			MetricKind:  m.metricDescriptorType(),
...
func (ma *metricAdapter) ensureMetricDescriptor(m Metric) error {
	if m.Unit == "" {
		return nil
	}
	...
	if _, ok := ma.descriptors[m.Name]; ok {
		return nil
	}

	err := ma.CreateMetricDescriptor(m)
	if err != nil {
		return err
	}
	ma.descriptors[m.Name] = struct{}{}
	return nil
}
